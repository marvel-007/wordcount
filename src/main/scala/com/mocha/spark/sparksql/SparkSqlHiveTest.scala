...
/*        while (true){
    ...
    def writeHiveTest(sc: SparkContext, sparkSession: SparkSession) = {
        ...
        //peopleListDF.write.mode(SaveMode.Append).saveAsTable("mochalog.logs_accesslog_1")
        peopleListDF.show()
        //peopleListDF.write.mode(SaveMode.Append).saveAsTable("mochalog.logs_parquet_1")
        //sparkSession.sql("DROP TABLE IF EXISTS logs_parquet")
        //sparkSession.sql("CREATE TABLE IF NOT EXISTS logs_parquet(name string,age int) STORED  AS PARQUET")
          sparkSession.sql("INSERT INTO mochalog.logs_accesslog_1 SELECT 'device_version' ,request ,'upstream_addr' ,body_bytes_sent,'browser_type' ," +
